...
export class AlertErrorComponent implements OnDestroy {
  ...
  constructor(private alertService: AlertService, private eventManager: EventManager) {
    this.errorListener = eventManager.subscribe('socialmediaplatformApp.error', (response: EventWithContent<unknown> | string) => {
      const errorResponse = (response as EventWithContent<AlertError>).content;
      this.addErrorAlert(errorResponse.message);
    });

    this.httpErrorListener = eventManager.subscribe('socialmediaplatformApp.httpError', (response: EventWithContent<unknown> | string) => {
  ...
  setClasses(alert: Alert): { [key: string]: boolean } {
    const classes = { 'mpl-toast': Boolean(alert.toast) };
    if (alert.position) {
